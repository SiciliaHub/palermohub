      ...
      style: {
        ...
        sources: {
          satellite: {
            ...
            tileSize: 256,
            attribution:
              "Immagini ©2025 Google.Immagini ©2025 AirbusCNES /Airbus.Maxar Technologies. Dati cartografici ©2025",
          },
        },
        layers: [
          {
...
function searchParticella() {
  ...
  if (features.length > 0) {
    ...
    new maplibregl.Popup()
      ...
      .setHTML(
        `<b>Particelle catastali</b><br><b>Foglio:</b> ${foglio}<br><b>Particella:</b> ${particella}`,
      )
      .addTo(map);
    ...
    errorDiv.textContent =
      "Nessuna particella trovata con i parametri specificati o la particella richiesta non è visibile nell'area corrente";
    pdfTab.style.display = "none";
    if (map.getLayer("highlighted-polygon")) {
...
function calculateAreasAndIntersections(features) {
  const particellaFeature = features.find(
    (f) => f.layer.id === "Particelle catastali",
  );
...
async function generatePDF() {
  ...
  if (isMobile && isPortrait) {
    alert(
      "Per generare il file PDF con le informazioni richieste, ruotare il dispositivo in orientamento orizzontale (modalità landscape).",
    );
    return;
  }
  ...
  const demoImageUrl =
    "https://palermohub.opendatasicilia.it/pmtiles/js/demo.png";
  const demoImageWidth = 35.28;
  const demoImageHeight = 18.7;
  doc.addImage(
    demoImageUrl,
    "PNG",
    pageWidth - margin - demoImageWidth,
    margin,
    demoImageWidth,
    demoImageHeight,
  );
  ...
  doc.setFontSize(16);
  doc.text(`Demo ricerca particella catastale`, pageWidth / 2, margin + 15, {
    align: "center",
  });
  ...
  doc.text(
    `Particella: ${window.currentParticella.particella}`,
    margin,
    margin + 35,
  );
  ...
  map.addLayer({
    ...
    filter: [
      "all",
      ["==", "Foglio", window.currentParticella.foglio],
      ["==", "Paricella", window.currentParticella.particella],
    ],
  });
  ...
  doc.setFontSize(8);
  doc.text(
    `Documento generato il ${now.toLocaleDateString()} alle ${now.toLocaleTimeString()}`,
    margin,
    margin + 45,
  );
  ...
  map.once("idle", async () => {
    try {
      ...
      if (!mapImage) {
        throw new Error(
          "Errore durante la generazione dell'immagine della mappa.",
        );
      }
      ...
      doc.text(disclaimerText, margin, yPos, {
      ...
      if (areaData) {
        yPos = addText(
          `• Area totale della particella: ${areaData.particellaArea} m²`,
          yPos + 5,
        );
        areaData.intersections.forEach((intersection) => {
          yPos = addText(
            `• Area intersecata con ${intersection.layer}: ${intersection.area} m² (${intersection.percentage}%)`,
            yPos + 3,
          );
        });
      }
      ...
      features.forEach((feature) => {
        ...
        switch (feature.layer.id) {
          case "cs":
            yPos = addText(
              `• Circoscrizione: ${feature.properties.Circoscriz}`,
              yPos + 3,
            );
            break;
          case "Info Vin. areali":
            ...
            yPos = addText(
              `  Descrizione: ${feature.properties.descrizone || "N/A"}`,
              yPos,
            );
            break;
          case "Info Vin. lineari":
            ...
            yPos = addText(`  Tipo: ${feature.properties.TIPO || "N/A"}`, yPos);
            yPos = addText(
              `  Descrizione: ${feature.properties.DESCRIZION || "N/A"}`,
              yPos,
            );
            break;
      ...
      doc.save(
        `particella_${window.currentParticella.foglio}_${window.currentParticella.particella}.pdf`,
      );
    } catch (error) {
...
const map = new maplibregl.Map({
  ...
  style: {
    version: 8,
    glyphs:
      "https://api.mapbox.com/fonts/v1/mapbox/{fontstack}/{range}.pbf?access_token=eyJ1IjoiZ2J2aXRyYW5vIiwiYSI6ImNtNWpwMDloejBtN3ozM3F3NzJvZGh2ZG4ifQ.AXXkYYL7XY6RBVXpJ2IrBA",
    sources: {
      "raster-tiles": {
        type: "raster",
...
map.on("load", () => {
  ...
  map.addSource("zto", {
    ...
    tileSize: 256,
    attribution:
      'Comune di Palermo - Variente Generle al P.R.G. 2004 - Rielaborazione di: <a href="https://twitter.com/opendatasicilia" target="_blank" title="opendatasicilia">@opendatasicilia</a>',
    minzoom: 12,
    maxzoom: 19,
  });
  ...
  const layers = [
    ...
    },
    {
      ...
      color: "#ffffff",
      tooltip:
        "<b>Tipo:</b> {{tipo}}<br><b>Descrizione:</b> {{descrizone}}<br><b>Note:</b> {{note}}",
    },
    {
      id: "Info Vin. lineari",
      source: "prg",
      ...
      color: "#ffffff",
      tooltip:
        "<b>Tipo:</b> {{TIPO}}<br><b>Descrizione:</b> {{DESCRIZION}}<br><b>Note:</b> {{note}}",
    },
    {
      ...
      color: "#ffffff",
      tooltip:
        "<b>Netto storico</b><br><b>ZTO:</b> {{ZTO}}<br><b>Descrizione:</b> {{DESCRIZION}}",
    },
    {
      id: "Info ZTO",
      source: "prg",
      ...
      color: "#ffffff",
      tooltip:
        "<b>Zonizzazione</b><br><b>ZTO:</b> {{ZTO}}<br><b>Descrizione:</b> {{DESCRIZION}}",
    },
    {
      ...
      color: "#ffffff",
      tooltip:
        "<b>Particelle catastali</b><br><b>Foglio:</b> {{Foglio}}<br><b>Particella:</b> {{Paricella}}",
    },
  ];
  ...
  layers.forEach((layer) => {
    map.addLayer({
      ...
      paint: {
        ...
        "fill-outline-color":
          layer.id === "Particelle catastali" ? "#000" : "transparent",
      },
    });
  });
...
map.on("click", (e) => {
  ...
  if (features.length > 0) {
    ...
    const unifiedTooltipContent = features
      .map((feature) => {
        let content = "";
