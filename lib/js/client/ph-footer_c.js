...
class FooterSmartSearch {
    ...
    async loadData() {
        try {
            ...
            // Fallback: prova il percorso locale
            try {
                console.log('Tentativo con percorso locale...');
                    ...
                    if (typeof Papa !== 'undefined') {
                        Papa.parse(csvText, {
                            ...
                            complete: (results) => {
                                ...
                                this.buildSearchIndex();
                                this.isInitialized = true;
                                console.log(`Footer Search (locale): Loaded ${this.allMaps.length} maps`);
                            }
                        });
                    } else {
                        this.parseCSVSimple(csvText);
...
// FUNZIONE CORRETTA per selezionare un suggerimento
// SOSTITUISCI SOLO la funzione selectFooterSuggestion con questa versione ULTRA-SEMPLICE:
...
// INOLTRE, controlla che il parsing CSV non stia rovinando i dati
// Nella classe FooterSmartSearch, nel metodo che usa PapaParse, 
// SOSTITUISCI la configurazione con questa più semplice:
...
Papa.parse(csvText, {
    ...
    delimiter: ',',
    complete: (results) => {
        console.log('Risultati parsing Papa:', results);
        this.allMaps = results.data.filter(row => row.titolo && row.titolo.trim() !== '');
        this.buildSearchIndex();
        this.isInitialized = true;
        console.log(`Footer Search: Loaded ${this.allMaps.length} maps`);
        
        // Debug: mostra i primi link-search
        if (this.allMaps.length > 0) {
            console.log('Campi CSV disponibili:', Object.keys(this.allMaps[0]));
            console.log('Primi 3 link-search:', this.allMaps.slice(0, 3).map(m => m['link-search']));
        }
    }
});
*/
