...
function initializeHeader() {
    ...
    // NUOVA FUNZIONE: Evidenzia la pagina attiva nel menu
...
function highlightActiveMenuPage() {
    ...
    // Mappa delle pagine per una corrispondenza più accurata
    const pageMapping = {
        'index.html': 'index.html',
        '': 'index.html', // Per il caso di root path
    ...
    // Rimuovi eventuali classi attive esistenti
    ...
    navLinks.forEach(link => {
        ...
        // Caso speciale per la home (può essere index.html, '', o '/')
            ...
            // Rimuovi attributi aria-current da link non attivi
            link.removeAttribute('aria-current');
        }
    });
    
    // Gestione speciale per pagine di mappe dinamiche
    // Se non troviamo corrispondenza nelle pagine statiche, 
    // potrebbe essere una pagina di mappa dinamica
    const activeLinks = document.querySelectorAll('.ph-nav-link.active');
    if (activeLinks.length === 0 && currentPage.endsWith('.html')) {
        // Se è una pagina HTML che non è nel menu, 
        // non evidenziare nessun elemento del menu
        console.log('Dynamic page detected, no menu highlighting:', currentPage);
    }
}

// ========= FUNZIONE PER AGGIORNARE LO STATO ATTIVO =========
function updateActiveMenuState() {
    // Questa funzione può essere chiamata quando si naviga dinamicamente
    highlightActiveMenuPage();
}
...
function addHeaderStyles() {
    ...
    styles.textContent = `
        ...
        /* NUOVO: Stile per il link della pagina attiva */
        .ph-nav-link.active {
            color: #ff9900;
        ...
        @media (max-width: 768px) {
            ...
            .ph-nav-link.active::after {
                display: none; /* Nascondi l'indicatore desktop su mobile */
            }
