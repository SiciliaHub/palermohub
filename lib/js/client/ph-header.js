...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Assicurati che l'header sia aperto all'avvio (solo su desktop/tablet)
...
function createModalHeader() {
    ...
    headerOverlay.style.display = 'none'; // Visibile solo su desktop via ensureHeaderOpen()

    // Crea l'header modale
    const modalHeader = document.createElement('div');
    modalHeader.id = 'modal-header';
    modalHeader.className = 'modal-header';
    modalHeader.style.display = 'none'; // Visibile solo su desktop via ensureHeaderOpen()
...
function openModalHeader() {
    ...
    if (closeHeaderTimerId) { clearTimeout(closeHeaderTimerId); closeHeaderTimerId = null; }
...
function closeModalHeader() {
    ...
    closeHeaderTimerId = setTimeout(() => {
        if (overlay) overlay.style.display = 'none';
        if (modalHeader) modalHeader.style.display = 'none';
        closeHeaderTimerId = null;
    }, 400);
...
function initializeBreadcrumbs() {
    loadMapsMetadata().then(() => {
        ...
        console.error('Breadcrumbs init fallita:', error);
    });
}
...
function updateBreadcrumbs() {
    ...
    if (!breadcrumbsEl || !breadcrumbsList) {
        console.error('Breadcrumbs: elementi #ph-breadcrumbs/#ph-breadcrumbs-list non trovati nel DOM');
        return;
    }
    ...
    try {
        ...
        console.error('Breadcrumbs: errore in updateBreadcrumbs():', error);
        // Fallback: mostra almeno "Home" invece di restare invisibile in silenzio
        breadcrumbsList.innerHTML = '<li><a href="index.html" class="ph-breadcrumb-link">Home</a></li>';
        breadcrumbsEl.style.display = 'block';
    }
}
...
function addModalHeaderStyles() {
    ...
    styles.textContent = `
        ...
        *:not(.map-card-inner), *::before, *::after {
