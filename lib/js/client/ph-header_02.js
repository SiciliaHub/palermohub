...
function displayMapMetadata(mapData) {
    ...
    // Crea "filtri" dai metadati della mappa (usando la struttura JSON reale)
    const metadataFields = [
        { key: 'fonte', label: 'Fonte', value: mapData.fonte },
        { key: 'anno', label: 'Anno', value: mapData.anno },
        { key: 'autore', label: 'Autore', value: mapData.autore },
        { key: 'categoria', label: 'Categoria', value: mapData.categoria },
        { key: 'territorio', label: 'Territorio', value: mapData.territorio }
    ];
    
    metadataFields.forEach(field => {
        if (field.value && field.value.toString().trim()) {
            metadataHTML += `
                <span class="ph-metadata-tag" data-type="${field.key}" title="${field.label}: ${field.value}">
                    ${field.label}: ${field.value.length > 20 ? field.value.substring(0, 20) + '...' : field.value}
                </span>
            `;
        }
    });
    
    // Aggiungi tags se presenti (nel JSON reale sono un array)
    if (mapData.tags && Array.isArray(mapData.tags) && mapData.tags.length > 0) {
        ...
        mapData.tags.slice(0, 3).forEach(tag => {
            if (tag && tag.trim()) {
                metadataHTML += `
                    <span class="ph-tag-chip" title="Tag: ${tag}">
                        ${tag}
                    </span>
                `;
            }
        ...
        if (mapData.tags.length > 3) {
            metadataHTML += `<span class="ph-more-tags" title="Altri tag: ${mapData.tags.slice(3).join(', ')}">+${mapData.tags.length - 3} altri</span>`;
        }
    }
...
function addHeaderStyles() {
    ...
    styles.textContent = `
        ...
        @media (max-width: 480px) {
            ...
            .ph-metadata-tag:nth-child(n+4),
            .ph-tag-chip:nth-child(n+4) {
                display: none;
            }
        }
