...
function selectFeaturedMaps() {
    ...
    const sortedMaps = [...allMaps].sort((a, b) => {
        const dateA = parseDate(a.data || a.anno || a.created || '');  // ← usa parseDate invece di parseInt
        const dateB = parseDate(b.data || b.anno || b.created || '');  // ← e data come prioritario
...
filteredMaps.sort((a, b) => {
    const dateA = parseDate(a.data || a.anno || a.created || '');  // ← data diventa prioritario
    const dateB = parseDate(b.data || b.anno || a.created || '');  // ← data diventa prioritario
...
// ordina le card e slider per data
function sortMapsByDate() {
    filteredMaps.sort((a, b) => {
        const dateA = parseDate(a.data || a.anno || a.created || '');  // ← data diventa prioritario
        const dateB = parseDate(b.data || b.anno || b.created || '');  // ← data diventa prioritario
