...
// JAVASCRIPT COMPLETO CON RESET AUTOMATICO ALLE IMPOSTAZIONI BASE

// ========= VARIABILI GLOBALI CORRETTE =========
let allMaps = [];
let filteredMaps = [];
const MAPS_COUNT_OFFSET = 1; // ponytail: conteggio "onesto+1" per arrotondare a 200, cresce da solo con le mappe vere
let isGridView = true;
...
// ========= FUNZIONE PARSEDATE MIGLIORATA =========

function parseDate(dateString) {
    if (!dateString || typeof dateString !== 'string') {
        console.warn('parseDate: Invalid input:', dateString);
        return new Date(0); // Data minima per ordinamento
    }
    
    const cleanDateString = dateString.trim();
    console.log(`parseDate: Processing "${cleanDateString}"`);
    
    // Se è solo un anno (es. "2025")
    if (/^\d{4}$/.test(cleanDateString)) {
        const year = parseInt(cleanDateString);
        console.log(`parseDate: Year only "${cleanDateString}" -> 1 Jan ${year}`);
        return new Date(year, 0, 1); // 1 gennaio dell'anno
    }
    
    // Formato DD-MM-YYYY (es. "06-09-2025")
    const ddmmyyyyMatch = cleanDateString.match(/^(\d{1,2})-(\d{1,2})-(\d{4})$/);
    if (ddmmyyyyMatch) {
        const day = parseInt(ddmmyyyyMatch[1]);
        const month = parseInt(ddmmyyyyMatch[2]) - 1; // JavaScript mesi da 0
        const year = parseInt(ddmmyyyyMatch[3]);
        
        // Validazione base
        if (month < 0 || month > 11 || day < 1 || day > 31) {
            console.warn(`parseDate: Invalid date components in "${cleanDateString}"`);
            return new Date(0);
        }
        
        const resultDate = new Date(year, month, day);
        console.log(`parseDate: DD-MM-YYYY "${cleanDateString}" -> ${resultDate.toDateString()}`);
        return resultDate;
    }
    
    // Formato DD/MM/YYYY
    const ddmmyyyySlashMatch = cleanDateString.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
    if (ddmmyyyySlashMatch) {
        const day = parseInt(ddmmyyyySlashMatch[1]);
        const month = parseInt(ddmmyyyySlashMatch[2]) - 1;
    ...
    if (yyyymmddMatch) {
        ...
        const resultDate = new Date(year, month, day);
        console.log(`parseDate: YYYY-MM-DD "${cleanDateString}" -> ${resultDate.toDateString()}`);
        return resultDate;
    }
    
    // Fallback: prova il parsing nativo di JavaScript
    const nativeDate = new Date(cleanDateString);
    if (!isNaN(nativeDate.getTime())) {
        console.log(`parseDate: Native parsing "${cleanDateString}" -> ${nativeDate.toDateString()}`);
        return nativeDate;
    }
    
    console.warn(`parseDate: Cannot parse date: "${cleanDateString}"`);
    return new Date(0); // Data minima se non riesce a parsare
}

// ========= FUNZIONE DEBUG PER DATE =========
function debugDates() {
    console.log("=== DEBUG DATE ===");
    const sampleMaps = allMaps.slice(0, 10);
    sampleMaps.forEach((map, index) => {
        const dataField = map.data || map.anno || '';
        const parsedDate = parseDate(dataField);
        console.log(`Map ${index}:`, {
            title: map.titolo || 'No title',
            originalData: map.data,
            originalAnno: map.anno,
            usedField: dataField,
            parsedDate: parsedDate,
            parsedString: parsedDate.toDateString(),
...
function selectFeaturedMaps() {
    ...
    console.log("=== SELECTING FEATURED MAPS ===");
    
    const sortedMaps = [...allMaps].sort((a, b) => {
        const dateA = parseDate(a.data || a.anno || '');
        const dateB = parseDate(b.data || b.anno || '');
        
        console.log(`Featured comparison: "${a.data || a.anno}" vs "${b.data || b.anno}"`);
        console.log(`Parsed: ${dateA.toDateString()} vs ${dateB.toDateString()}`);
        
        return dateB.getTime() - dateA.getTime(); // Dal più recente al più vecchio
    });

    const selected = sortedMaps.slice(0, 5);
    console.log("Selected featured maps:", selected.map(m => ({ 
        title: m.titolo, 
        date: m.data || m.anno,
        parsed: parseDate(m.data || m.anno || '').toDateString()
    })));
...
// ========= RICERCA INTELLIGENTE MIGLIORATA CON ESPANSIONE E RESET =========
...
// ========= FUNZIONI RESET E GESTIONE ESPANSIONE =========

function resetToBaseSettings() {
    console.log('[RESET] Resetting all settings to base configuration');
    ...
    // Reset ordinamento al default (dal più recente al più vecchio)
    sortOrder = 'desc';
    updateSortIndicator();
    
    // Reset paginazione alla prima pagina
    currentPage = 1;
    
    // Reset modalità visualizzazione a griglia (opzionale)
    if (!isGridView) {
        isGridView = true;
        const button = document.getElementById('view-toggle');
        if (button) {
            button.innerHTML = '<i class="fas fa-list"></i> Lista';
        }
    }
    
    // Applica i filtri reset (mostrerà tutte le mappe)
    applyFilters();
    
    console.log('[RESET] Base settings restored - showing all maps');
}

function showResetFeedback() {
    // Mostra brevemente un feedback che tutto è stato resettato
    const statusElement = document.getElementById('search-status');
    if (statusElement) {
        statusElement.innerHTML = '<i class="fas fa-check"></i> Ricerca cancellata - Visualizzazione completa ripristinata';
        statusElement.className = 'search-status reset-feedback';
...
function handleEscapeKey() {
    ...
    if (filtersSection && filtersSection.classList.contains('search-active')) {
        console.log('[SEARCH] ESC pressed - clearing search and resetting');
        
        // Pulisci completamente la ricerca
        clearSmartSearch();
        
        // Rimuovi focus dal campo di ricerca
        if (searchInput) searchInput.blur();
        
        // Scroll alla sezione mappe per mostrare i risultati reset
        setTimeout(() => {
...
function handleSearchFocus() {
    ...
    if (searchInput && searchInput.value.trim().length >= 2) {
        if (filtersSection) {
            filtersSection.classList.add('search-active', 'expanded');
        }
        // Rimostra i suggerimenti se ci sono
...
function handleSearchBlur() {
    // Non rimuovere l'espansione immediatamente per permettere il click sui suggerimenti
    setTimeout(() => {
        const searchInput = document.getElementById('smart-search-input');
        const hasSearchText = searchInput && searchInput.value.trim().length >= 2;
        
        if (!hasSearchText) {
            const filtersSection = document.querySelector('.filters-section');
            if (filtersSection) {
                filtersSection.classList.remove('search-active', 'expanded');
            }
            
            // NUOVO: Se il campo è completamente vuoto, reset alle impostazioni base
            if (searchInput && searchInput.value.trim() === '') {
                console.log('[SEARCH] Field empty on blur - resetting to base settings');
                resetToBaseSettings();
                showResetFeedback();
            }
        }
...
function handleSearchResize() {
    ...
    if (filtersSection && filtersSection.classList.contains('search-active')) {
        // Calcola l'altezza ottimale basata sui suggerimenti
        if (suggestionsContainer && suggestionsContainer.classList.contains('visible')) {
            ...
            if (suggestionsList) {
                ...
                const optimalHeight = Math.min(450 + (itemCount * 120), 1000);
                filtersSection.style.maxHeight = optimalHeight + 'px';
            }
        }
    }
}

// ========= FUNZIONI RICERCA MODIFICATE CON ESPANSIONE E RESET =========

// ========================================
//   SOLUZIONE SEMPLICE: ESPANSIONE RICERCA
//   Sostituisci solo queste funzioni nel file home.js esistente
// ========================================

// 1. Modifica handleSmartSearchInput per gestire l'espansione
function handleSmartSearchInput(event) {
    ...
    if (query.length < 2) {
        hideSuggestions();
        // NUOVO: Rimuovi espansione quando non c'è ricerca
        if (filtersSection) {
            filtersSection.classList.remove('search-active', 'expanded');
        }
        applyFilters();
    ...
    // NUOVO: Espandi la sezione quando c'è ricerca
    if (filtersSection) {
        filtersSection.classList.add('search-active', 'expanded');
    }
...
// 2. Modifica clearSmartSearch per rimuovere l'espansione
function clearSmartSearch() {
    ...
    // NUOVO: Rimuovi espansione quando si cancella la ricerca
    if (filtersSection) {
        filtersSection.classList.remove('search-active', 'expanded');
    }
...
// 3. Modifica hideSuggestions per gestire la contrazione
function hideSuggestions() {
    ...
    // NUOVO: Rimuovi espansione solo se non c'è testo di ricerca
    const searchInput = document.getElementById('smart-search-input');
    if (searchInput && searchInput.value.trim().length < 2) {
        if (filtersSection) {
            filtersSection.classList.remove('search-active', 'expanded');
...
// 4. Modifica displaySmartSuggestions per assicurare l'espansione
function displaySmartSuggestions(results, query) {
    ...
    // NUOVO: Assicurati che la sezione sia espansa
    if (filtersSection) {
        filtersSection.classList.add('search-active', 'expanded');
    }
...
// 5. Aggiungi gestione ESC per chiudere la ricerca
...
function displaySmartSuggestions(results, query) {
    ...
    // Assicurati che la filters-section sia espansa
    if (filtersSection) {
        filtersSection.classList.add('search-active', 'expanded');
    }
    ...
    // Scroll automatico alla sezione ricerca quando si espande
    setTimeout(() => {
        const filtersSection = document.querySelector('.filters-section');
        if (filtersSection) {
...
function hideSuggestions() {
    ...
    // Se non c'era nulla di aperto (nessun suggerimento, nessuna ricerca attiva),
    // non c'e' nulla da "chiudere": evita di richiamare applyFilters()/displayMaps()
    // ad ogni singolo click sulla pagina (es. click sulle card flip), che altrimenti
    // ricrea tutte le card e annulla lo stato "flipped" appena impostato dal click.
    const wasActive = (suggestionsContainer && suggestionsContainer.classList.contains('visible')) ||
                       (filtersSection && filtersSection.classList.contains('search-active'));

    if (suggestionsContainer) {
        suggestionsContainer.classList.remove('visible');
    }

    // Controlla se c'è ancora testo di ricerca
    const searchInput = document.getElementById('smart-search-input');
    const hasSearchText = searchInput && searchInput.value.trim().length >= 2;

    // Rimuovi la classe search-active solo se non c'è testo di ricerca
    if (!hasSearchText) {
        if (filtersSection) {
            filtersSection.classList.remove('search-active', 'expanded');
        }

        // Se non c'è più ricerca attiva, applica un soft reset
        if (wasActive && searchInput && searchInput.value.trim() === '') {
            // Solo riapplica i filtri senza reset completo
            applyFilters();
        }
    }
...
function clearSmartSearch() {
    console.log('[SEARCH] Clearing search and resetting to base settings');
    ...
    // Nascondi suggerimenti e rimuovi espansione
    hideSuggestions();
    if (filtersSection) {
        filtersSection.classList.remove('search-active', 'expanded');
    }
    
    // Reset completo alle impostazioni di base
    resetToBaseSettings();
    showResetFeedback();
}
    ...
    try {
        ...
        Papa.parse(csvText, {
            ...
            complete: function(results) {
                ...
                // Debug delle date dopo il caricamento
                setTimeout(debugDates, 500);
...
function updateMapCount() {
    ...
    if (mapCountEl) {
        const isUnfiltered = filteredMaps.length === allMaps.length;
        const shownCount = isUnfiltered ? filteredMaps.length + MAPS_COUNT_OFFSET : filteredMaps.length;
        mapCountEl.textContent = '🗺️ ' + shownCount + ' di ' + (allMaps.length + MAPS_COUNT_OFFSET) + ' mappe';
    }
}
...
function resetAllFilters() {
    console.log('[FILTERS] Resetting all filters including search');
...
// ponytail: replica minmax(280px,1fr)+gap 2rem del CSS .maps-grid, tenere in sync se cambia
function getGridColumns(container) {
    if (!isGridView || !container) return 1;
    const cardMin = 280, gap = 32;
...
function displayMaps() {
    ...
    // Elementi per pagina arrotondati al multiplo delle colonne visibili: ultima riga sempre piena
    const gridColumns = getGridColumns(container);
    const effectiveItemsPerPage = (currentViewMode === 'pagination' && isGridView && gridColumns > 1)
        ? Math.ceil(itemsPerPage / gridColumns) * gridColumns
        : itemsPerPage;
    ...
    if (currentViewMode === 'pagination') {
        ...
        totalPages = Math.ceil(filteredMaps.length / effectiveItemsPerPage);
        console.log(`[DISPLAY] Total pages calculated: ${totalPages} (${filteredMaps.length} maps / ${effectiveItemsPerPage} per page)`);
        ...
        let startIndex = (currentPage - 1) * effectiveItemsPerPage;
        let endIndex = startIndex + effectiveItemsPerPage;
        ...
        if (startIndex >= filteredMaps.length) {
            ...
            currentPage = 1;
            startIndex = 0;
            endIndex = effectiveItemsPerPage;
        }
    ...
    container.innerHTML = mapsToDisplay.map((map, index) => {
        ...
        if (isGridView) {
            return `<div class="map-card map-card-flip"
                         data-map-url="${mapUrl}"
                         data-map-data="${mapDataEncoded}"
                         data-map-index="${index}">
              <div class="map-card-inner">
                <div class="map-card-face map-card-front">
                  <img src="${imageUrl}"
                       alt="${safeTitle}"
                       class="map-image"
                       onerror="this.src='${fallbackUrl}';">
                  <div class="map-content">
                      ...
                      <div class="map-card-footer">
                          <div class="map-flip-hint"><i class="fas fa-sync-alt"></i> Clicca per i dettagli</div>
                          <button type="button" class="map-open-btn"><i class="fas fa-external-link-alt"></i> Apri mappa</button>
                      </div>
                  </div>
                </div>
                <div class="map-card-face map-card-back">
                  <div class="map-content">
                      <h3 class="map-title">${safeTitle}</h3>
    ...
    cards.forEach((card, index) => {
        ...
        card.addEventListener('click', function(event) {
            // Ignora click su tag e author (filtrano, non aprono/girano la card)
            if (event.target.closest('.meta-tag, .author-tag')) {
                return;
            }

            // Card "flip" (vista griglia): il click gira la card avanti/indietro,
            // solo il pulsante "Apri mappa" (fronte o retro, a seconda di quale
            // faccia e' visibile) naviga davvero.
            // Nota: non si puo' usare event.target.closest('.map-open-btn') qui.
            // Le due facce sono ruotate in 3D (rotateY 180 sul genitore +
            // rotateY 180 sulla faccia stessa): Chromium non riesce a fare
            // hit-test dentro questo doppio contesto 3D e restituisce sempre
            // la card come target, mai il bottone, anche quando il bottone e'
            // visibile e cliccabile. Si verifica quindi la posizione del click
            // contro il rettangolo reale del bottone visibile.
            if (card.classList.contains('map-card-flip')) {
                const visibleFace = card.classList.contains('flipped') ? '.map-card-back' : '.map-card-front';
                const openBtnEl = card.querySelector(`${visibleFace} .map-open-btn`);
                const btnRect = openBtnEl ? openBtnEl.getBoundingClientRect() : null;
                const clickedOpenBtn = btnRect &&
                    event.clientX >= btnRect.left && event.clientX <= btnRect.right &&
...
// ========= FUNZIONE APPLYFILTERS MODIFICATA CON ORDINAMENTO CORRETTO =========
function applyFilters() {
    ...
    // Controlla se tutti i filtri sono vuoti (stato base)
    const isBaseState = Object.values(filterValues).every(value => value === '');
    
    if (isBaseState) {
        console.log('[FILTERS] In base state - showing all maps');
    }
    ...
    filteredMaps.sort((a, b) => {
        ...
        if (sortOrder === 'desc') {
            ...
            return dateA.getTime() - dateB.getTime(); // Dal più vecchio al più recente
        }
    });
    ...
    // Se siamo in stato base, mostra feedback
    if (isBaseState && filteredMaps.length === allMaps.length) {
        console.log('[FILTERS] Base state confirmed - all maps visible');
    }
}

function sortMapsByDate() {
    console.log("=== MANUAL SORTING MAPS ===");
    
    filteredMaps.sort((a, b) => {
        const dateA = parseDate(a.data || a.anno || '');
        const dateB = parseDate(b.data || b.anno || '');
        
        console.log(`Manual sort: "${a.data || a.anno}" vs "${b.data || b.anno}"`);
        console.log(`Parsed: ${dateA.toDateString()} vs ${dateB.toDateString()}`);
    ...
    console.log("First 5 after manual sorting:", filteredMaps.slice(0, 5).map(m => ({ 
        title: m.titolo, 
        date: m.data || m.anno,
        parsed: parseDate(m.data || m.anno || '').toString()
    })));
    
    // NON resettare currentPage quando si ordina manualmente
    displayMaps();
    updateSortIndicator();
}
...
// ========= INIZIALIZZAZIONE EVENT LISTENERS CORRETTA CON RESET AUTOMATICO =========
document.addEventListener('DOMContentLoaded', function() {
    console.log('[INIT] Initializing PalermoHub with automatic reset on search close');
    ...
    // Ricerca intelligente CON GESTIONE ESPANSIONE E RESET
    const searchInput = document.getElementById('smart-search-input');
    ...
    if (searchInput) {
        ...
        // Gestione focus/blur per l'espansione e reset
    ...
    // Ricalcola colonne griglia al resize (ultima riga sempre piena). displayMaps()
    // ricrea tutte le card da zero, quindi va richiamato SOLO se il numero di
    // colonne e' davvero cambiato: un resize minimo (es. comparsa/scomparsa della
    // scrollbar) altrimenti ricostruiva la griglia inutilmente e cancellava lo
    // stato "flipped" delle card appena girate dall'utente.
    let gridResizeTimeout;
    let lastGridColumnsCount = getGridColumns(document.getElementById('maps-container'));
    window.addEventListener('resize', () => {
        clearTimeout(gridResizeTimeout);
        gridResizeTimeout = setTimeout(() => {
            if (currentViewMode !== 'pagination' || !isGridView) return;
            const container = document.getElementById('maps-container');
            const cols = getGridColumns(container);
            if (cols === lastGridColumnsCount) return;
            lastGridColumnsCount = cols;
            displayMaps();
        }, 200);
    });
    ...
    // Gestione ESC per chiudere la ricerca espansa con reset completo
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Escape') {
            handleEscapeKey();
        }
    ...
    window.debugDates = debugDates; // Esponi debug date
    window.resetToBaseSettings = resetToBaseSettings; // Esponi funzione reset
    ...
    console.log('[INIT] PalermoHub initialization completed with automatic reset functionality');
});