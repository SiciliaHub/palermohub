...
function displayMaps() {
    ...
    if (currentViewMode === 'pagination') {
        ...
        // MODALITÀ PAGINAZIONE - LOGICA CORRETTA
        ...
        // Validazione pagina corrente - PIÙ RIGOROSA
        if (totalPages > 0) {
        ...
        // Calcola mappe da visualizzare per la pagina corrente - CON DEBUG
        ...
        if (startIndex >= filteredMaps.length) {
            ...
            // Forza il reset alla prima pagina
            currentPage = 1;
            startIndex = 0;
        ...
        // Aggiorna controlli paginazione PRIMA del rendering
        updatePaginationControls();
        
    } else {
        // MODALITÀ SCROLL INFINITO
        console.log('[DISPLAY] Processing infinite scroll mode');
        mapsToDisplay = filteredMaps;
        totalPages = 1;
    }
    
    // Aggiorna visibilità controlli
    updatePaginationVisibility();

     // ========= RENDERING DELLE MAPPE - INVARIATO =========
    ...
    container.innerHTML = mapsToDisplay.map((map, index) => {
        ...
        const mapUrl = map.url || map.URL || '';

        if (isGridView) {
            return `<div class="map-card" data-map-url="${mapUrl}" data-map-data='${mapDataJson}'>
                <img src="${imageUrl}" 
                     alt="${map.titolo}" 
                     class="map-image" 
                     onerror="this.src='${fallbackUrl}';">
    ...
    // ========= EVENT LISTENERS DELLE MAPPE - INVARIATO =========
    container.querySelectorAll('.map-card').forEach(card => {
        card.addEventListener('click', function(event) {
            ...
            const mapDataStr = this.dataset.mapData;
            
            if (!mapUrl || !mapDataStr) return;
            
            try {
                const mapData = JSON.parse(mapDataStr);
                handleMapClick(event, mapUrl, mapData);
            } catch (e) {
                console.error('Errore nel parsing dei dati mappa:', e);
            }
        });
...
function sortMapsByDate() {
    ...
    // NON resettare currentPage quando si ordina
    displayMaps();
    updateSortIndicator();
}
