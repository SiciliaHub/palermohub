...
// FUNZIONE CORRETTA - USA LA COLONNA "link-search"
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        // USA LA COLONNA "link-search" (posizione 14 del CSV)
        const mapUrl = result.map['link-search'];
        
        console.log('Campo link-search:', JSON.stringify(mapUrl));
        
        if (mapUrl && typeof mapUrl === 'string' && mapUrl.trim() !== '') {
            const cleanUrl = mapUrl.trim();
            
            console.log('URL da aprire:', cleanUrl);
            
            // REGOLA: link palermohub.opendatasicilia.it = stessa finestra, altri = nuova finestra
            if (cleanUrl.includes('palermohub.opendatasicilia.it')) {
                console.log('Link interno PalermoHub - stessa finestra');
            ...
            console.error('Campo link-search vuoto per:', result.map.titolo);
            console.log('Debug mappa completa:', result.map);
            alert(`Link non disponibile per: ${result.map.titolo || 'questa mappa'}`);
        }
        
        hideFooterSuggestions();
    } else {
        console.error('Indice suggerimento non valido:', index);
    }
}
