...
// NUOVA FUNZIONE: Chiama la funzione di ricerca esistente in home.js
async function performFooterSearch(query) {
    try {
        // Verifica se la funzione searchMaps esiste (solo nell'index)
        if (typeof searchMaps === 'function') {
            console.log("Chiamata a searchMaps in home.js per la query:", query);
            const results = searchMaps(query);
            console.log("Risultati ricevuti da searchMaps:", results.length);
            displayFooterSuggestions(results, query);
        } else {
            // Se non siamo nell'index, reindirizza alla home con il parametro di ricerca
            console.log('Reindirizzamento alla home per la ricerca:', query);
...
function displayFooterSuggestions(results, query) {
    ...
    if (results.length === 0) {
        ...
        suggestionsList.innerHTML = results.slice(0, 6).map((result, index) => {
            // Usa i campi corretti dal CSV come definiti in home.js
            const titolo = result.titolo || 'N/A';
            const descrizione = result.descrizione || '';
            const territorio = result.territorio || 'N/A';
            const linkSearch = result['link-search'] || '#';
            
            // Estrai il tipo di match (se disponibile)
            const matchType = result.matchType || 'Generico';
            
            // Funzione semplice per evidenziare il testo (puoi migliorarla)
            const highlightText = (text, query) => {
                if (!text || !query) return text;
                const regex = new RegExp(`(${query})`, 'gi');
                return text.replace(regex, '<span class="footer-highlight">$1</span>');
            };
            
            const highlightedTitle = highlightText(titolo, query);
            const highlightedDescription = highlightText(descrizione, query);
            
            // Funzione per ottenere l'immagine (semplificata)
            const getImageUrl = (result) => {
                // Prova diversi campi possibili per l'immagine
                const imageFields = ['img', 'image', 'immagine', 'foto', 'picture'];
                for (const field of imageFields) {
                    if (result[field] && result[field].trim() !== '') {
                        let url = result[field].trim();
                        // Se è un URL relativo, costruisci l'URL completo
                        if (!url.startsWith('http') && !url.startsWith('/')) {
                            url = './' + url; // Assumi che sia nella stessa directory
                        }
                        return url;
                    }
                }
                ...
                return 'data:image/svg+xml,' + encodeURIComponent(
                    ...
                    'fill="#657786" text-anchor="middle" dy=".3em">No img</text></svg>'
                );
            };
            ...
            return `<div class="footer-suggestion-item" data-index="${index}">
                ...
                <div class="footer-suggestion-content">
                    ...
                    <div class="footer-suggestion-meta">
                        ...
                        <span class="footer-suggestion-tag">${territorio}</span>
                    </div>
                </div>
            </div>`;
        }).join('');
...
// NUOVA FUNZIONE: Gestisce la selezione di un suggerimento
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        console.log('=== DEBUG SELEZIONE MAPPA FOOTER ===');
        console.log('Mappa selezionata:', result.titolo);
        console.log('Link da aprire:', linkSearch);
        
        if (linkSearch && typeof linkSearch === 'string' && linkSearch.trim() !== '') {
            const cleanUrl = linkSearch.trim();
            
            // Apri il link - usa sempre una nuova finestra per coerenza
            window.open(cleanUrl, '_blank');
        } else {
            console.error('Link non disponibile per:', result.titolo);
            alert(`Link non disponibile per: ${result.titolo || 'questa mappa'}`);
        }
        hideFooterSuggestions();
    } else {
...
// Carica gli stili per la ricerca nel footer (mantieni il CSS esistente)
...
// ========================================
// SISTEMA PRIVACY E GDPR (mantieni il codice esistente)
// ========================================
...
// ========================================
// FUNZIONI SOCIAL SHARING (mantieni il codice esistente)
// ========================================
