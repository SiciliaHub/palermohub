...
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        console.log('Campo URL dal CSV:', result.map.url || result.map.URL || 'N/A');
        
        // Recupera l'URL dal CSV (controlla sia minuscolo che maiuscolo)
        let mapUrl = result.map.url || result.map.URL;
        
        if (mapUrl && mapUrl.trim() !== '') {
            // Pulizia robusta dell'URL
            mapUrl = String(mapUrl)
                .trim()
                .replace(/^[\"'`\\s]+|[\"'`\\s]+$/g, '') // Rimuove virgolette e spazi
            ...
            // LOGICA CORRETTA:
            // - Se è un URL completo (http:// o https://) → lascialo invariato
            // - Se è un URL relativo → aggiungi il dominio corrente
            let finalUrl;
            if (mapUrl.startsWith('http://') || mapUrl.startsWith('https://')) {
                ...
                let cleanPath = mapUrl.startsWith('/') ? mapUrl.substring(1) : mapUrl;
