...
class FooterSmartSearch {
    ...
    async loadData() {
        try {
            ...
            const csvFilePath = 'https://raw.githubusercontent.com/SiciliaHub/palermohub/PH-release-v2.0/dati-palermo-hub/palermo-hub.csv';
...
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        // SOLUZIONE SEMPLIFICATA: Usa direttamente la colonna URL dal CSV
        ...
        if (mapUrl && mapUrl.trim() !== '') {
            ...
            if (!mapUrl.startsWith('http://') && !mapUrl.startsWith('https://')) {
                // File HTML locale - costruisci il percorso completo
                mapUrl = window.location.origin + window.location.pathname.replace(/\/[^/]*$/, '/') + mapUrl;
            }
            // Altrimenti è già un URL completo esterno, lascialo così com'è
