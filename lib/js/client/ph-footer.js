...
class FooterSmartSearch {
    ...
    async loadData() {
        try {
            ...
            console.log('Footer Search: Caricamento CSV da:', csvUrl);
            ...
            if (typeof Papa !== 'undefined') {
                Papa.parse(csvText, {
                    ...
                    complete: (results) => {
                        ...
                        console.log(`Footer Search: Caricate ${this.allMaps.length} mappe`);
                        
                        // Debug: mostra i campi disponibili
                        if (this.allMaps.length > 0) {
                            console.log('Campi CSV disponibili:', Object.keys(this.allMaps[0]));
                            // Mostra alcuni esempi di URL
                            console.log('Esempi di URL:');
                            for (let i = 0; i < Math.min(3, this.allMaps.length); i++) {
                                console.log(`${this.allMaps[i].titolo}: URL="${this.allMaps[i].URL || 'N/A'}"`);
                            }
                        }
                    }
                });
    ...
    parseCSVSimple(csvText) {
        ...
        this.buildSearchIndex();
        this.isInitialized = true;
        console.log(`Footer Search: Caricate ${this.allMaps.length} mappe (simple parser)`);
    }
...
// FUNZIONE PRINCIPALE PER LA GESTIONE DEI LINK - VERSIONE CORRETTA
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        console.log('URL dal CSV:', result.map.URL);
        console.log('Pagina corrente:', window.location.href);
        
        // Estrai l'URL dalla colonna 'URL' (maiuscolo)
        let mapUrl = result.map.URL;
        
        if (!mapUrl || mapUrl.trim() === '') {
            console.error('URL non disponibile per:', result.map.titolo);
            alert(`Link non disponibile per: ${result.map.titolo}`);
            hideFooterSuggestions();
            return;
        }
        ...
        mapUrl = String(mapUrl)
            .trim()
            .replace(/^[\"'`<>\\s]+|[\"'`<>\\s]+$/g, '') // Rimuove caratteri speciali all'inizio/fine
            .replace(/[\x00-\x1F\x7F]/g, '') // Rimuove caratteri di controllo
            .replace(/\s+/g, ' '); // Rimuove spazi multipli
        
        console.log('URL dopo pulizia:', mapUrl);
        
        // Determina se è un URL completo o relativo
        let finalUrl;
        if (mapUrl.startsWith('http://') || mapUrl.startsWith('https://')) {
            // URL completo esterno - usalo così com'è
            finalUrl = mapUrl;
            console.log('URL completo esterno:', finalUrl);
        } else {
            // URL relativo - dobbiamo costruire l'URL assoluto rispetto alla radice del sito
            
            // 1. Ottieni il dominio completo (protocollo + dominio)
            const domain = window.location.origin;
            
            // 2. Rimuovi eventuali slash iniziali dal percorso relativo
            let cleanPath = mapUrl.replace(/^\/+/, '');
            
            // 3. Costruisci l'URL completo: dominio + percorso relativo
            finalUrl = domain + '/' + cleanPath;
            
            console.log('URL relativo convertito:', finalUrl);
            console.log('Dominio usato:', domain);
            console.log('Pulito percorso:', cleanPath);
        }
        
        // Apri l'URL nella stessa finestra
        console.log('Apertura URL nella stessa finestra:', finalUrl);
...
function getFooterImageUrl(mapData, index) {
    ...
    if (!cleanUrl.match(/^https?:\/\//)) {
        // Percorso sempre relativo alla cartella legend/clip_index/
...
function shareOnBluesky() {
    const url = encodeURIComponent(window.location.href);
    const title = encodeURIComponent(document.title);
    window.open(`https://bsky.app/intent/compose?text=${title}%20${url}`, '_blank');
}