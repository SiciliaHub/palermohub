...
// Funzione per detectare se siamo su mobile
function isMobileDevice() {
    return /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) 
           || window.innerWidth <= 768;
}

// Funzione per forzare la visibilità del pulsante su mobile
function ensureFooterButtonVisibility() {
    const toggleTab = document.getElementById('footer-toggle-tab');
    if (!toggleTab) return;
    
    // Forza gli stili per mobile
    if (window.innerWidth <= 768) {
        toggleTab.style.position = 'fixed';
        toggleTab.style.zIndex = '99999';
        toggleTab.style.display = 'flex';
        toggleTab.style.visibility = 'visible';
...
// Funzione di debug per verificare la posizione del pulsante
function debugFooterButton() {
    const toggleTab = document.getElementById('footer-toggle-tab');
    if (!toggleTab) {
        console.log('❌ Footer toggle button NOT found');
        return;
    }
    
    const rect = toggleTab.getBoundingClientRect();
    const styles = window.getComputedStyle(toggleTab);
    
    console.log('🔍 Footer Button Debug Info:');
    ...
    console.log('Bottom:', styles.bottom, 'Right:', styles.right);
    
    // Verifica se è visibile nel viewport
    ...
    // Aggiunge classe debug per evidenziarlo
    toggleTab.classList.add('debug');
    setTimeout(() => {
        toggleTab.classList.remove('debug');
    }, 3000);
}

// Listener per resize e orientamento
function handleMobileResize() {
    ensureFooterButtonVisibility();
    
    // Forza reflow
    const toggleTab = document.getElementById('footer-toggle-tab');
    if (toggleTab) {
        toggleTab.style.display = 'none';
        toggleTab.offsetHeight; // Trigger reflow
        toggleTab.style.display = 'flex';
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ...
        // Debug automatico su mobile (rimuovere in produzione)
...
function createFooterToggleTab() {
    ...
    // Aggiunta classe per mobile
    if (isMobileDevice()) {
        toggleTab.classList.add('mobile-visible');
    }
    ...
    if (isMobileDevice()) {
        toggleTab.addEventListener('touchstart', function(e) {
            e.preventDefault();
            this.style.transform = 'scale(0.95)';
        });
        
        toggleTab.addEventListener('touchend', function(e) {
            e.preventDefault();
    ...
    // Forza la visibilità dopo un breve delay
    setTimeout(() => {
        ensureFooterButtonVisibility();
    }, 500);
}
        ...
        style.innerHTML = `
            ...
            @keyframes mobileAttention {
                ...
                50% {
                    ...
                    box-shadow: 0 6px 25px rgba(255, 153, 0, 0.7);
                }
            }
            ...
            @media (max-width: 768px) {
                .footer-toggle-tab {
                    ...
                    height: 34px;
                    z-index: 99999;
                    animation: mobileAttention 3s ease-in-out infinite;
                }
            }
            ...
            [data-theme="dark"] .footer-toggle-tab {
                border: 2px solid rgba(255, 255, 255, 0.6);
                box-shadow: 0 4px 20px rgba(255, 167, 38, 0.5);
            }
            
            .footer-toggle-tab.debug {
...
// ========================================
// EVENT LISTENERS AGGIUNTIVI PER MOBILE
// ========================================
...
// Listener per quando la pagina diventa visibile
document.addEventListener('visibilitychange', () => {
    if (!document.hidden) {
        setTimeout(ensureFooterButtonVisibility, 100);
    }
});

// Forza il button dopo il caricamento di immagini
window.addEventListener('load', () => {
    setTimeout(ensureFooterButtonVisibility, 500);
});

// Fix per iOS Safari
if (/iPad|iPhone|iPod/.test(navigator.userAgent)) {
    // Attende che iOS Safari finisca il rendering
    setTimeout(() => {
        ensureFooterButtonVisibility();
        ...
        const toggleTab = document.getElementById('footer-toggle-tab');
        if (toggleTab) {
            toggleTab.style.webkitTransform = 'translateZ(0)';
            toggleTab.style.webkitBackfaceVisibility = 'hidden';
        }
    }, 1500);
}
...
window.testFooterButton = function() {
    debugFooterButton();
    ensureFooterButtonVisibility();
    console.log('Footer button test completed. Check console for details.');
};

// Debug functions per sviluppo
window.debugFooterButton = debugFooterButton;
window.ensureFooterButtonVisibility = ensureFooterButtonVisibility;