...
// FUNZIONE PRINCIPALE PER LA GESTIONE DEI LINK - VERSIONE CON 'link-search'
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        console.log('Link dal CSV:', result.map['link-search']);
        
        // Estrai l'URL dalla colonna 'link-search'
        ...
        const domain = 'https://palermohub.opendatasicilia.it/';
        const isSameDomain = mapUrl.startsWith(domain);
        
        console.log('Dominio di riferimento:', domain);
        console.log('Link appartiene allo stesso dominio:', isSameDomain);
        
        if (isSameDomain) {
            // Link dello stesso dominio - apri nella stessa finestra
            console.log('Apertura nella stessa finestra:', mapUrl);
            window.location.href = mapUrl;
        } else {
            // Link esterno - apri in una nuova finestra
            console.log('Apertura in nuova finestra:', mapUrl);
