...
async loadData() {
    try {
        // CORREZIONE: Usa percorso assoluto dalla root del sito
        // Invece di: const csvFilePath = 'dati-palermo-hub/palermo-hub.csv';
        
        // Determina il percorso corretto dalla root del sito
        const csvFilePath = this.getCorrectCSVPath();
        ...
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}: ${response.statusText} - URL: ${csvFilePath}`);
        }
        ...
        if (typeof Papa !== 'undefined') {
            Papa.parse(csvText, {
                ...
                complete: (results) => {
                    ...
                    console.log(`Footer Search: Loaded ${this.allMaps.length} maps from ${csvFilePath}`);
                }
            });
        } else {
