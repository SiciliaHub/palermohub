...
class FooterSmartSearch {
    ...
    // Determina l'URL base del sito
    getBaseURL() {
        const currentDomain = window.location.origin;
        const currentPath = window.location.pathname;
        
        console.log('Calcolo baseURL - Domain:', currentDomain, 'Path:', currentPath);
        
        // Se siamo su GitHub Pages (es: https://siciliahub.github.io/palermohub/...)
        if (currentDomain.includes('github.io')) {
            // Estrae il nome del repository dal path
            const pathParts = currentPath.split('/').filter(part => part !== '');
            if (pathParts.length > 0) {
                // Il primo elemento è sempre il nome del repository
                const repoName = pathParts[0];
                const baseURL = `${currentDomain}/${repoName}`;
                console.log('GitHub Pages - Repository:', repoName, 'BaseURL:', baseURL);
                return baseURL;
            }
            return currentDomain;
        }
        
        // Per domini personalizzati (es: https://palermohub.opendatasicilia.it)
        // I file sono nella radice, quindi il base URL è solo il dominio
        console.log('Dominio personalizzato - BaseURL:', currentDomain);
        return currentDomain;
    }

    async loadData() {
        try {
            // URL CORRETTO GitHub con refs/heads/
            ...
            console.log('Base URL del sito:', this.baseURL);
            ...
            if (typeof Papa !== 'undefined') {
                Papa.parse(csvText, {
                    ...
                    complete: (results) => {
                        ...
                        // Debug: mostra i campi disponibili e alcuni esempi
                        if (this.allMaps.length > 0) {
                            console.log('=== CSV DEBUG ===');
                            console.log('Campi CSV disponibili:', Object.keys(this.allMaps[0]));
                            
                            // Mostra i primi 3 esempi con focus sul campo URL
                            this.allMaps.slice(0, 3).forEach((map, i) => {
                                console.log(`Esempio ${i+1}:`);
                                console.log(`  Titolo: "${map.titolo}"`);
                                console.log(`  url (minuscolo): "${map.url}"`);
                                console.log(`  URL (maiuscolo): "${map.URL}"`);
                                console.log(`  Tutti i campi:`, map);
                                console.log('---');
                            });
                            console.log('===============');
                        }
    ...
    // Costruisce l'URL corretto per la navigazione
    buildMapURL(mapData) {
        // Il campo URL nel CSV può essere "url" o "URL" a seconda del parsing
        let mapUrl = mapData.url || mapData.URL || '';
        
        if (!mapUrl || mapUrl.trim() === '') {
            console.warn('URL mappa vuoto per:', mapData.titolo);
            return null;
        }

        // Pulizia dell'URL
        mapUrl = mapUrl.trim().replace(/^"+|"+$/g, '').trim();

        // Se è già un URL completo, utilizzalo direttamente
        if (mapUrl.startsWith('http://') || mapUrl.startsWith('https://')) {
            return mapUrl;
        }

        // Se inizia con #, è un anchor nella pagina corrente
        if (mapUrl.startsWith('#')) {
            return mapUrl;
        }

        // Se inizia con /, è un percorso assoluto dal dominio
        if (mapUrl.startsWith('/')) {
            return this.baseURL + mapUrl;
        }

        // Percorso relativo - file nella radice del repository
        // Assicurati che baseURL non sia vuoto
        if (!this.baseURL || this.baseURL.trim() === '') {
            console.error('Base URL non definito. Utilizzo del dominio corrente.');
            this.baseURL = window.location.origin;
        }
        
        // Costruisci l'URL completo per file nella radice
        // Assicurati che non ci siano doppi slash
        const cleanBaseURL = this.baseURL.endsWith('/') ? this.baseURL.slice(0, -1) : this.baseURL;
        const fullURL = `${cleanBaseURL}/${mapUrl}`;
        
        console.log(`Costruzione URL: BaseURL="${this.baseURL}", MapURL="${mapUrl}", Result="${fullURL}"`);
        return fullURL;
    }
}
...
function displayFooterSuggestions(results, query) {
    ...
    footerSearchSuggestions = results; // CORRETTO: usa footerSearchSuggestions
...
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        const selectedResult = footerSearchSuggestions[index];
        const selectedMap = selectedResult.map;
        
        // Debug logging per capire cosa sta succedendo
        console.log('=== DEBUG SELECTION ===');
        console.log('Index selezionato:', index);
        console.log('Risultato completo:', selectedResult);
        console.log('Dati mappa:', selectedMap);
        console.log('Campo url:', selectedMap.url);
        console.log('Campo URL:', selectedMap.URL);
        console.log('Tutti i campi della mappa:', Object.keys(selectedMap));
        
        const mapUrl = footerSmartSearch.buildMapURL(selectedMap);
        
        console.log('URL costruito dalla buildMapURL:', mapUrl);
        console.log('========================');
        ...
        if (mapUrl && mapUrl.trim()) {
            ...
            try {
                console.log('Navigando verso:', mapUrl);
                window.location.href = mapUrl;
            } catch (error) {
                console.error('Errore durante la navigazione:', error);
                if (searchInput) {
                    searchInput.placeholder = 'Errore nel caricamento';
                }
            }
        } else {
            console.warn('URL mappa non valido o vuoto per:', selectedMap.titolo);
        }
    }
