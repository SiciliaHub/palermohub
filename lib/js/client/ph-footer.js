...
class FooterSmartSearch {
    ...
    async loadData() {
        try {
            // CORREZIONE: URL completo GitHub invece di percorso relativo
...
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        // Debug: mostra tutti i dati disponibili
        console.log('Debug mappa selezionata:', result.map);
        console.log('Campi disponibili:', Object.keys(result.map));
        
        // PRIMA cerca nel campo 'link-search', poi negli altri campi come fallback
        let mapUrl = '';
        
        // Priorità 1: Campo 'link-search' (il nuovo campo con i link corretti)
        if (result.map['link-search'] && result.map['link-search'].trim() !== '') {
            mapUrl = result.map['link-search'].trim();
            console.log(`URL trovato nel campo link-search: ${mapUrl}`);
        } 
        // Fallback: cerca negli altri campi possibili
        else {
            ...
            // Fallback finale: cerca qualsiasi campo che contenga "http"
            if (!mapUrl) {
        ...
        if (mapUrl) {
            // Assicurati che inizi con http/https se non è già presente
            ...
            // LOGICA PER APRIRE I LINK:
            // - Link di palermohub.opendatasicilia.it -> stessa finestra
            // - Altri link -> nuova finestra
            if (mapUrl.includes('palermohub.opendatasicilia.it')) {
                // Link interno - apri nella stessa finestra
                window.location.href = mapUrl;
            } else {
                // Link esterno - apri in nuova finestra
