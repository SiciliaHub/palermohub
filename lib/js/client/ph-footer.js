...
// ========================================
// RICERCA INTELLIGENTE NEL FOOTER - VERSIONE MIGLIORATA
// ========================================

// Classe SmartSearch migliorata per il footer (copiata da home.js e adattata)
class FooterSmartSearch {
    ...
    async loadData() {
        try {
            // Determina il percorso corretto del CSV in base alla pagina corrente
            const csvFilePath = this.determineCsvPath();
            
            console.log('Footer Search: Caricamento CSV da:', csvFilePath);
    ...
    determineCsvPath() {
        ...
        // Se siamo nell'index o nella root, usa il percorso diretto
        if (!currentPage || currentPage === 'index.html' || currentPath === '/' || currentPath === '') {
            return 'dati-palermo-hub/palermo-hub.csv';
        }
        
        // Per qualsiasi altra pagina (incluse quelle nella root come "pagina.html"), 
        // il CSV è sempre nello stesso posto relativo
        return 'dati-palermo-hub/palermo-hub.csv';
    }
            ...
            return `<div class="footer-suggestion-item" data-index="${index}">
                ...
                <div class="footer-suggestion-content">
                    ...
                    <div class="footer-suggestion-meta">
                        ...
                        <span class="footer-suggestion-tag">${result.map.anno || 'N/A'}</span>
                    </div>
                </div>
                <div class="footer-suggestion-relevance ${relevanceClass}" 
                     title="Rilevanza: ${Math.round(result.relevance * 100)}%"></div>
            </div>`;
        }).join('');
    }
...
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        console.log('Debug mappa selezionata:', result.map);
        console.log('Campo URL:', result.map.url);
        
        // Usa direttamente la colonna URL dal CSV
        let mapUrl = result.map.url;
        
        if (mapUrl && mapUrl.trim() !== '') {
            // Pulizia robusta dell'URL - rimuove virgolette e caratteri indesiderati
            mapUrl = String(mapUrl)
                .trim()
                .replace(/^["'`\s]+|["'`\s]+$/g, '') // Rimuove virgolette all'inizio e alla fine
                .replace(/[\r\n\t]/g, '') // Rimuove caratteri di controllo
                .replace(/[^\x20-\x7E]/g, ''); // Rimuove caratteri non ASCII
            
            console.log('URL pulito:', mapUrl);
            
            // Determina il percorso corretto per l'apertura della mappa
            const finalUrl = this.determineFinalMapUrl(mapUrl);
            
            console.log('Apertura URL finale:', finalUrl);
            window.open(finalUrl, '_blank');
        } else {
            console.error('URL non disponibile per:', result.map.titolo || 'questa mappa');
...
// Determina l'URL finale corretto per l'apertura della mappa
function determineFinalMapUrl(mapUrl) {
    // Se l'URL è completo (inizia con http/https), lascialo invariato
    if (mapUrl.startsWith('http://') || mapUrl.startsWith('https://')) {
        return mapUrl;
    }
    
    // Se è un file HTML relativo, costruisci il percorso completo
    const currentPath = window.location.pathname;
    const currentPage = currentPath.split('/').pop();
    
    // Sempre torna alla root del sito per le mappe
    let basePath = window.location.origin;
    
    // Se siamo nell'index/root, usa il percorso corrente
    if (!currentPage || currentPage === 'index.html' || currentPath === '/' || currentPath === '') {
        basePath += '/';
    } 
    // Per tutte le altre pagine, torna sempre alla root
    else {
        basePath += '/';
    }
    
    return basePath + mapUrl;
}
...
function getFooterImageUrl(mapData, index) {
    ...
    if (!cleanUrl.match(/^https?:\/\//)) {
        // Determina il percorso corretto per le immagini
        const imagePath = this.determineImagePath(cleanUrl);
        return imagePath;
    }
...
// Determina il percorso corretto per le immagini
function determineImagePath(cleanUrl) {
    const currentPath = window.location.pathname;
    const currentPage = currentPath.split('/').pop();
    const isInSubfolder = currentPath.includes('/') && currentPath !== '/' && !currentPath.endsWith('index.html') && currentPage !== '';
    
    const fileName = cleanUrl.split('/').pop();
    
    // Se siamo nella root o nell'index
    if (!currentPage || currentPage === 'index.html' || currentPath === '/') {
        return 'legend/clip_index/' + fileName;
    } 
    // Se siamo in una sottocartella o file HTML, vai alla directory superiore
    else if (isInSubfolder || currentPage.endsWith('.html')) {
        return '../legend/clip_index/' + fileName;
    }
...
function loadFooterSearchStyles() {
    ...
    if (!document.getElementById(cssId)) {
        ...
        style.innerHTML = `
            ...
            .footer-suggestion-relevance.high {
                background: #48bb78;
            }

            .footer-suggestion-relevance.medium {
                background: #ed8936;
            }

            .footer-suggestion-relevance.low {
                background: #a0aec0;
            }
