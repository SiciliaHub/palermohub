...
class FooterSmartSearch {
    ...
    // Costruisce l'URL corretto per la navigazione - VERSIONE SEMPLIFICATA
    buildMapURL(mapData) {
        // Prende il campo URL dal CSV (maiuscolo o minuscolo)
        let urlFromCSV = mapData.URL || mapData.url || '';
        
        if (!urlFromCSV || urlFromCSV.trim() === '') {
            console.warn('URL vuoto nel CSV per:', mapData.titolo);
            return null;
        }

        // Pulisce l'URL da spazi e virgolette
        urlFromCSV = urlFromCSV.trim().replace(/^"+|"+$/g, '');

        // Se è già un URL completo (con http/https), usalo direttamente
        if (urlFromCSV.startsWith('http://') || urlFromCSV.startsWith('https://')) {
            return urlFromCSV;
        }

        // Altrimenti è un file relativo: dominio + / + filename
        return this.baseURL + '/' + urlFromCSV;
    }
}
