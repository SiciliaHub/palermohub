...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Avvia l'effetto pulse dopo un breve delay (solo per 3 secondi)
    setTimeout(() => {
        startFooterPulseFor3Seconds();
    }, 1500);
});
...
function openModalFooter() {
    ...
    if (closeFooterTimerId) { clearTimeout(closeFooterTimerId); closeFooterTimerId = null; }
...
function closeModalFooter() {
    ...
    closeFooterTimerId = setTimeout(() => {
        if (overlay) overlay.style.display = 'none';
        if (modalFooter) modalFooter.style.display = 'none';
        closeFooterTimerId = null;
        // Riavvia l'effetto pulse dopo la chiusura (solo per 3 secondi)
        startFooterPulseFor3Seconds();
    }, 400);
...
// ========================================
// EFFETTO PULSE FOOTER (3 SECONDI) - DEBUG VERSION
// ========================================

function startFooterPulseFor3Seconds() {
    const toggleTab = document.getElementById('footer-toggle-tab');
    console.log('startFooterPulseFor3Seconds chiamata:', toggleTab, 'isFooterOpen:', isFooterOpen);
    
    if (toggleTab && !isFooterOpen) {
        console.log('Applicando classe footer-pulse-active');
        toggleTab.classList.add('footer-pulse-active');
        
        // Verifica che la classe sia stata applicata
        setTimeout(() => {
            console.log('Classi elemento:', toggleTab.className);
            console.log('Ha classe pulse?', toggleTab.classList.contains('footer-pulse-active'));
        }, 100);
        
        // Ferma l'animazione dopo 3 secondi
        setTimeout(() => {
            console.log('Fermando pulse dopo 3 secondi');
            stopFooterPulse();
        }, 3000);
    } else {
        console.log('Pulse non avviato - elemento:', !!toggleTab, 'footer chiuso:', !isFooterOpen);
    }
}

function stopFooterPulse() {
    const toggleTab = document.getElementById('footer-toggle-tab');
    if (toggleTab) {
        console.log('Rimuovendo classe footer-pulse-active');
        toggleTab.classList.remove('footer-pulse-active');
    }
}
...
function loadFooterModalStyles() {
    ...
    if (!document.getElementById(cssId)) {
        ...
        style.innerHTML = `
            ...
            /* EFFETTO PULSE - VERSIONE ROBUSTA CHE SUPERA I CONFLITTI */
            .footer-toggle-tab.footer-pulse-active {
                animation: footerPulse 1.5s ease-out 2 !important;
                transition: none !important; /* Disabilita transitions durante pulse */
            }
            
            /* Disabilita hover durante pulse */
            .footer-toggle-tab.footer-pulse-active:hover {
                animation: footerPulse 1.5s ease-out 2 !important;
            ...
            .footer-toggle-tab:hover.footer-pulse-active {
                animation: none; /* Ferma il pulse in hover */
            }
            ...
            .footer-toggle-tab.tab-active {
                ...
                width: 52px;
                height: 36px;
                animation: none !important; /* Ferma il pulse quando attivo */
            }
