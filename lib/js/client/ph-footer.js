                ...
                Papa.parse(csvText, {
                    ...
                    complete: (results) => {
                        ...
                        if (this.allMaps.length > 0) {
                            ...
                            // Mostra alcuni esempi di URL per debugging
                            this.allMaps.slice(0, 3).forEach((map, i) => {
                                console.log(`Esempio ${i+1} - Titolo: "${map.titolo}", URL: "${map.url}"`, map);
                            });
                        }
                    }
                });
...
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) { // CORRETTO
        const selectedMap = footerSearchSuggestions[index].map; // CORRETTO
        const mapUrl = footerSmartSearch.buildMapURL(selectedMap); // Usa la funzione corretta
        
        console.log(`🗺️ Footer: Navigazione a "${selectedMap.titolo}"`);
        console.log(`🔗 Footer: URL costruito: ${mapUrl}`);
        ...
        if (mapUrl && mapUrl.trim()) {
            ...
            console.warn('⚠️ Footer: URL mappa non valido o vuoto');
        }
    }
