...
class FooterSmartSearch {
    ...
    async loadData() {
        try {
            // Percorso sempre relativo alla root
...
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        console.log('Mappa selezionata:', result.map);
        console.log('Tutti i campi disponibili:', Object.keys(result.map));
        
        // Mostra tutti i valori che potrebbero essere URL
        const possibleUrlFields = ['url', 'URL', 'link', 'Link', 'href', 'mappa', 'file'];
        console.log('Valori dei campi potenziali:');
        possibleUrlFields.forEach(field => {
            if (result.map[field] !== undefined) {
                console.log(`- ${field}: "${result.map[field]}" (tipo: ${typeof result.map[field]})`);
            }
        });
        
        // Cerca un campo che contenga un URL valido
        let mapUrl = null;
        let foundField = null;
        
        // Prima cerca campi specifici per l'URL
        for (const field of ['url', 'URL', 'link', 'Link', 'href']) {
            if (result.map[field] && typeof result.map[field] === 'string' && result.map[field].trim() !== '') {
                mapUrl = result.map[field];
                foundField = field;
                break;
            }
        }
        
        // Se non trovato, cerca qualsiasi campo che assomigli a un URL
        if (!mapUrl) {
            for (const [key, value] of Object.entries(result.map)) {
                if (typeof value === 'string' && value.trim() !== '' && 
                    (value.includes('.html') || value.includes('.htm') || 
                     value.startsWith('http://') || value.startsWith('https://') ||
                     value.includes('/') || value.includes('.php'))) {
                    mapUrl = value;
                    foundField = key;
                    break;
                }
        ...
        console.log(`URL trovato nel campo "${foundField}":`, mapUrl);
        
        if (!mapUrl || mapUrl.trim() === '') {
            console.error('Nessun URL valido trovato per questa mappa');
            alert(`Nessun link disponibile per: ${result.map.titolo || 'questa mappa'}`);
            hideFooterSuggestions();
            return;
        }
        
        // Pulizia avanzata dell'URL
        mapUrl = String(mapUrl)
            .trim()
            // Rimuove virgolette, apici e caratteri speciali all'inizio e alla fine
            .replace(/^[\"'`<>\\s]+|[\"'`<>\\s]+$/g, '')
        ...
        // Verifica se è un URL completo o relativo
        let finalUrl;
        if (mapUrl.startsWith('http://') || mapUrl.startsWith('https://')) {
            // URL completo esterno
            finalUrl = mapUrl;
            console.log('URL completo esterno:', finalUrl);
        } else {
            // URL relativo - aggiungi il dominio corrente
            // Rimuovi slash iniziali per evitare doppi slash
            let cleanPath = mapUrl.replace(/^\/+/, '');
            finalUrl = window.location.origin + '/' + cleanPath;
            console.log('URL relativo convertito in:', finalUrl);
        }
        
        // Validazione finale dell'URL
        try {
            new URL(finalUrl);
            console.log('URL valido:', finalUrl);
            
            // Apri l'URL in una nuova scheda
            window.open(finalUrl, '_blank');
        } catch (e) {
            console.error('URL non valido:', finalUrl, e);
            alert(`Link non valido per: ${result.map.titolo || 'questa mappa'}\nURL: ${finalUrl}`);
        }
