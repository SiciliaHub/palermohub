...
// ========================================
// RICERCA INTELLIGENTE NEL FOOTER - VERSIONE CORRETTA
// ========================================
...
class FooterSmartSearch {
    ...
    async loadData() {
        console.log('=== DEBUG CARICAMENTO CSV ===');
        ...
        const csvUrls = [
            // Prima prova: URL GitHub assoluto (SEMPRE funziona)
            'https://raw.githubusercontent.com/SiciliaHub/palermohub/refs/heads/PH-release-v2.0/dati-palermo-hub/palermo-hub.csv',
            
            // Seconda prova: percorso assoluto dal root
            '/dati-palermo-hub/palermo-hub.csv',
            
            // Terza prova: percorso relativo (solo per index)
            'dati-palermo-hub/palermo-hub.csv',
            
            // Quarta prova: un livello su (per pagine in sottocartelle)
            '../dati-palermo-hub/palermo-hub.csv'
        ];
        
        for (let i = 0; i < csvUrls.length; i++) {
            const csvUrl = csvUrls[i];
            console.log(`\nTentativo ${i + 1}: ${csvUrl}`);
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    console.log(`CSV caricato! Lunghezza: ${csvText.length} caratteri`);
                    ...
                    if (typeof Papa !== 'undefined') {
                        console.log('Usando PapaParse...');
                        Papa.parse(csvText, {
                            header: true,
                            skipEmptyLines: true,
                            delimiter: ',',
                            complete: (results) => {
                                console.log('Parse completato:', results.data.length, 'righe');
                                this.allMaps = results.data.filter(row => row.titolo && row.titolo.trim() !== '');
                                
                                console.log(`Mappe filtrate: ${this.allMaps.length}`);
                                
                                // Debug dei primi dati
                                if (this.allMaps.length > 0) {
                                    console.log('Campi disponibili:', Object.keys(this.allMaps[0]));
                                    console.log('Test link-search primi 3:');
                                    this.allMaps.slice(0, 3).forEach((map, idx) => {
                                        console.log(`  ${idx + 1}: "${map['link-search']}"`);
                                    });
                                }
                                
                                this.buildSearchIndex();
                                this.isInitialized = true;
                                console.log('✅ Footer Search inizializzato con successo!');
                            }
                        });
                    } else {
                        console.log('PapaParse non disponibile, usando parser semplice...');
                ...
                console.error(`❌ Errore con ${csvUrl}:`, error.message);
            }
        }
        
        console.error('❌ TUTTI i tentativi di caricamento CSV falliti!');
        console.log('Verifica che il file esista e sia accessibile.');
    }
...
// FUNZIONE CORRETTA per selezionare un suggerimento - VERSIONE SEMPLIFICATA
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        console.log('=== DEBUG SELEZIONE MAPPA ===');
        console.log('Mappa selezionata:', result.map.titolo);
        console.log('Campi disponibili:', Object.keys(result.map));
        
        // Leggi il campo 'link-search' senza modifiche
        let mapUrl = result.map['link-search'];
        
        console.log('Campo link-search RAW:', JSON.stringify(mapUrl));
        
        if (mapUrl && typeof mapUrl === 'string') {
            // SOLO trim - niente altre modifiche
            mapUrl = mapUrl.trim();
            
            console.log('Link dopo trim:', JSON.stringify(mapUrl));
            console.log('Inizia con https?', mapUrl.startsWith('https://'));
            console.log('Contiene palermohub?', mapUrl.includes('palermohub.opendatasicilia.it'));
            
            if (mapUrl.length > 0) {
                console.log('Apertura URL:', mapUrl);
                
                // LOGICA APERTURA FINESTRE (senza modificare l'URL):
                if (mapUrl.includes('palermohub.opendatasicilia.it')) {
                    console.log('Link interno - stessa finestra');
                    window.location.href = mapUrl;
                } else {
                    console.log('Link esterno - nuova finestra');
                    window.open(mapUrl, '_blank');
                }
            } else {
                console.error('Link vuoto dopo trim');
                alert(`Link vuoto per: ${result.map.titolo || 'questa mappa'}`);
            }
        } else {
            console.error('Campo link-search non trovato o non valido:', mapUrl);
            alert(`Campo link-search non disponibile per: ${result.map.titolo || 'questa mappa'}\nTipo: ${typeof mapUrl}\nValore: ${mapUrl}`);
        }
...
function testFooterSearchData() {
    console.log('=== TEST DATI RICERCA FOOTER ===');
    
    if (!footerSmartSearch || !footerSmartSearch.isInitialized) {
        console.error('❌ Ricerca non inizializzata');
        return;
    }
    
    console.log(`✅ Mappe caricate: ${footerSmartSearch.allMaps.length}`);
    
    if (footerSmartSearch.allMaps.length > 0) {
        const firstMap = footerSmartSearch.allMaps[0];
        console.log('Prima mappa:', firstMap.titolo);
        console.log('Link-search:', JSON.stringify(firstMap['link-search']));
        console.log('Ha link-search?', !!firstMap['link-search']);
        
        // Test ricerca
        const testResults = footerSmartSearch.search('palermo');
        console.log(`Test ricerca "palermo": ${testResults.length} risultati`);
    }
}
