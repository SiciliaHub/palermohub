...
function initFooterMapSearch() {
    setTimeout(() => {
        ...
        // Prova a popolare subito se i dati sono già disponibili
...
function performFooterSearch(query) {
    if (!footerAllMaps || footerAllMaps.length === 0) {
        // Prova a prendere i dati da window.allMaps se disponibili
        if (window.allMaps && window.allMaps.length > 0) {
            footerAllMaps = [...window.allMaps];
        } else {
            console.log('⚠️ Nessuna mappa disponibile per la ricerca footer');
            return;
        }
    }
...
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchResults.length) {
        ...
        if (mapUrl) {
            console.log(`🗺️ Footer navigation to: ${selectedMap.titolo}`);
...
// Aggiorna i dati quando vengono ricaricati
