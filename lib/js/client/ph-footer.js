                ...
                <div class="footer-search-wrapper">
                    <div class="footer-search-input-container" id="footer-search-input-container">
                        ...
                        <input type="text" 
                               ...
                               placeholder="Cerca mappe per titolo, descrizione, autore, territorio..."
                               autocomplete="off"
                               autocorrect="off"
                               spellcheck="false">
...
function initFooterSearch() {
    ...
    // Carica i dati CSV
    loadFooterSearchData();
    
    // Imposta gli event listeners
    setupFooterSearchEvents();
}

// Carica i dati CSV per la ricerca
async function loadFooterSearchData() {
    try {
        // URL del CSV aggiornato
        const csvUrl = 'https://raw.githubusercontent.com/SiciliaHub/palermohub/refs/heads/PH-release-v2.0/dati-palermo-hub/palermo-hub.csv';
        
        console.log('Caricamento CSV da:', csvUrl);
        ...
        // Parse del CSV
        footerMapsData = parseFooterCSV(csvText);
        footerSearchInitialized = true;
        
        console.log(`Footer Search: ${footerMapsData.length} mappe caricate`);
        
        // Mostra un esempio dei dati
        if (footerMapsData.length > 0) {
            console.log('Esempio prima mappa:', footerMapsData[0]);
            console.log('Campi disponibili:', Object.keys(footerMapsData[0]));
        }
        
    } catch (error) {
        console.error('Errore caricamento CSV footer:', error);
        
        // Fallback: prova percorso locale
        try {
            const localResponse = await fetch('dati-palermo-hub/palermo-hub.csv');
            if (localResponse.ok) {
                const csvText = await localResponse.text();
                footerMapsData = parseFooterCSV(csvText);
                footerSearchInitialized = true;
                console.log(`Footer Search (locale): ${footerMapsData.length} mappe caricate`);
            }
        } catch (localError) {
            console.error('Errore anche con percorso locale:', localError);
        }
    }
}

// Parse semplice del CSV
function parseFooterCSV(csvText) {
    const lines = csvText.trim().split('\n');
    if (lines.length < 2) return [];
    
    // Prendi l'header e puliscilo
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        // Parsing più robusto per gestire virgole dentro le virgolette
        const values = parseCSVLine(line);
        
        if (values.length >= headers.length - 1) { // Permetti una certa tolleranza
            const row = {};
            headers.forEach((header, index) => {
                row[header] = (values[index] || '').trim().replace(/"/g, '');
            });
            
            // Verifica che abbia almeno un titolo
...
// Parse di una singola linea CSV gestendo virgolette
function parseCSVLine(line) {
    ...
    result.push(current); // Aggiungi l'ultimo valore
    return result;
}

// Imposta gli event listeners per la ricerca
function setupFooterSearchEvents() {
    ...
    if (!searchInput) {
        console.error('Footer search input non trovato');
        return;
    }
    ...
    searchInput.addEventListener('input', function(event) {
        ...
        // Mostra/nasconde il pulsante clear
        if (clearBtn) {
    ...
    // Keyboard navigation
    searchInput.addEventListener('keydown', handleFooterKeyboard);
...
function performFooterSearch(query) {
    if (!footerSearchInitialized || footerMapsData.length === 0) {
        console.log('Ricerca non inizializzata o nessun dato disponibile');
        hideFooterSuggestions();
        return;
    }
    ...
    try {
        const results = searchFooterMaps(query);
        displayFooterResults(results, query);
    } catch (error) {
...
// Funzione di ricerca
function searchFooterMaps(query) {
    const queryLower = query.toLowerCase();
    const queryWords = queryLower.split(/\s+/).filter(w => w.length > 1);
    
    const results = [];
    
    footerMapsData.forEach((map, index) => {
        let score = 0;
        const matchedFields = [];
        
        // Campi da cercare con i loro pesi
        const searchFields = {
            'titolo': 10,
            'descrizione': 5,
            'territorio': 8,
        ...
        // Calcola il punteggio per ogni campo
        Object.entries(searchFields).forEach(([field, weight]) => {
            const fieldValue = (map[field] || '').toLowerCase();
            if (!fieldValue) return;
            
            // Ricerca esatta della query completa
            if (fieldValue.includes(queryLower)) {
                score += weight * 2;
                matchedFields.push(field);
            }
            
            // Ricerca delle singole parole
        ...
        // Se ha un punteggio, aggiungilo ai risultati
        if (score > 0) {
            results.push({
                map: map,
                score: score,
    ...
    // Ordina per punteggio decrescente e limita a 6 risultati
    results.sort((a, b) => b.score - a.score);
    return results.slice(0, 6);
}

// Mostra i risultati
function displayFooterResults(results, query) {
    ...
    if (!suggestionsContainer || !suggestionsList || !suggestionsCount) {
        console.error('Elementi UI per i suggerimenti non trovati');
        return;
    }
    
    footerCurrentResults = results;
    ...
    if (results.length === 0) {
        suggestionsList.innerHTML = `
            <div class="footer-no-results">
                <div class="icon"><i class="fas fa-search"></i></div>
                <div>Nessun risultato trovato per "<strong>${escapeHtml(query)}</strong>"</div>
            </div>
        `;
    } else {
        suggestionsList.innerHTML = results.map((result, index) => {
            const map = result.map;
            const imageUrl = getFooterMapImage(map);
            const highlightedTitle = highlightText(map.titolo || '', query);
            const highlightedDescription = highlightText(map.descrizione || '', query);
            const matchType = getMatchType(result.matchedFields);
            
            return `
                <div class="footer-suggestion-item" data-index="${index}">
                    <img src="${imageUrl}" 
                         alt="${escapeHtml(map.titolo || '')}" 
                         class="footer-suggestion-preview"
                         onerror="this.style.display='none'">
                    <div class="footer-suggestion-content">
                        <div class="footer-suggestion-title">${highlightedTitle}</div>
                        <div class="footer-suggestion-description">${highlightedDescription}</div>
                        <div class="footer-suggestion-meta">
                            <span class="footer-match-type">${matchType}</span>
                            <span class="footer-suggestion-tag">${escapeHtml(map.territorio || 'N/A')}</span>
                        </div>
                    </div>
                </div>
            `;
    ...
    suggestionsList.querySelectorAll('.footer-suggestion-item').forEach((item, index) => {
        item.addEventListener('click', () => {
            selectFooterResult(index);
        });
    });
}

// Determina il tipo di match
function getMatchType(matchedFields) {
    if (matchedFields.includes('titolo')) return 'Titolo';
    if (matchedFields.includes('descrizione')) return 'Descrizione';
    if (matchedFields.includes('territorio')) return 'Territorio';
    if (matchedFields.includes('collaborazione')) return 'Autore';
    if (matchedFields.includes('categoria')) return 'Categoria';
    if (matchedFields.includes('tag')) return 'Tag';
    return 'Generico';
}

// Seleziona un risultato
function selectFooterResult(index) {
    if (index < 0 || index >= footerCurrentResults.length) {
        console.error('Indice risultato non valido:', index);
        return;
    }
    
    const result = footerCurrentResults[index];
    const map = result.map;
    
    console.log('Mappa selezionata:', map);
    
    // Cerca il link nel campo 'link-search' (priorità assoluta)
    let mapUrl = '';
    
    if (map['link-search'] && map['link-search'].trim()) {
        mapUrl = map['link-search'].trim();
        console.log('Link trovato in link-search:', mapUrl);
    } else {
        // Fallback: altri possibili campi
        const urlFields = ['url', 'URL', 'link', 'href', 'mappa'];
        for (const field of urlFields) {
            if (map[field] && map[field].trim()) {
                mapUrl = map[field].trim();
                console.log(`Link trovato in ${field}:`, mapUrl);
                break;
            }
        }
    }
    ...
    if (mapUrl) {
        ...
        console.log('Apertura URL finale:', mapUrl);
        
        // Determina se aprire nella stessa finestra o in una nuova
        if (mapUrl.includes('palermohub.opendatasicilia.it')) {
            // Link interno - stessa finestra
            window.location.href = mapUrl;
        } else {
            // Link esterno - nuova finestra
            window.open(mapUrl, '_blank', 'noopener,noreferrer');
        }
    } else {
        alert(`Link non disponibile per: ${map.titolo || 'questa mappa'}`);
        console.error('Nessun link trovato per la mappa:', map);
    }
...
function handleFooterKeyboard(event) {
    const suggestionsContainer = document.getElementById('footer-search-suggestions');
    ...
    switch (event.key) {
        case 'ArrowDown':
            ...
            footerSelectedSuggestion = Math.min(
                footerSelectedSuggestion + 1, 
                footerCurrentResults.length - 1
            );
        ...
        case 'Enter':
            ...
            if (footerSelectedSuggestion >= 0) {
                selectFooterResult(footerSelectedSuggestion);
            } else if (footerCurrentResults.length > 0) {
                selectFooterResult(0);
            }
            break;
...
function updateFooterSelection() {
    ...
    // Scroll dell'elemento selezionato
    if (footerSelectedSuggestion >= 0 && items[footerSelectedSuggestion]) {
        items[footerSelectedSuggestion].scrollIntoView({
            block: 'nearest',
            behavior: 'smooth'
        });
    }
...
// Ottiene l'immagine per una mappa
function getFooterMapImage(map) {
    const imageFields = ['img', 'image', 'immagine', 'foto'];
    
    for (const field of imageFields) {
        if (map[field] && map[field].trim()) {
            let imageUrl = map[field].trim().replace(/^["'\s]+|["'\s]+$/g, '');
            
            if (imageUrl.startsWith('http')) {
                return imageUrl;
            } else {
                // Percorso relativo
                return `legend/clip_index/${imageUrl}`;
            }
        }
    }
    
    // Immagine di fallback
    return createFooterPlaceholderImage();
}

// Crea un'immagine placeholder
function createFooterPlaceholderImage() {
    const svg = `<svg width="60" height="40" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="50%" y="50%" font-family="Arial,sans-serif" font-size="8" 
              fill="#657786" text-anchor="middle" dy=".3em">No img</text>
    </svg>`;
    return 'data:image/svg+xml,' + encodeURIComponent(svg);
}

// Evidenzia il testo della ricerca
function highlightText(text, query) {
    ...
    queryWords.forEach(word => {
        if (word.length >= 2) {
            ...
            highlighted = highlighted.replace(regex, '<span class="footer-highlight">$1</span>');
        }
    });
