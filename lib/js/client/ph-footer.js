...
// CORREZIONE del percorso CSV nel file ph-footer.js
// Trova il metodo loadData() nella classe FooterSmartSearch e sostituisci questa parte:

// CORREZIONE per CSV ospitato su GitHub
// Sostituisci il metodo loadData() nella classe FooterSmartSearch con questo:

async loadData() {
    try {
        // URL completo del CSV su GitHub
        const csvFilePath = 'https://raw.githubusercontent.com/SiciliaHub/palermohub/refs/heads/PH-release-v2.0/dati-palermo-hub/palermo-hub.csv';
		        
        console.log('Caricamento CSV da GitHub:', csvFilePath);
        ...
        if (typeof Papa !== 'undefined') {
            Papa.parse(csvText, {
                ...
                complete: (results) => {
                    ...
                    console.log(`Footer Search: Loaded ${this.allMaps.length} maps from GitHub`);
                    
                    // Debug: mostra i primi dati per verificare la struttura
                    if (this.allMaps.length > 0) {
                        console.log('Campi disponibili:', Object.keys(this.allMaps[0]));
                        console.log('Esempio di mappa:', this.allMaps[0]);
                    }
                }
            });
        } else {
        ...
        console.error('Errore nel caricamento dati da GitHub per footer search:', error);
        
        // Fallback: prova URL alternativi
        this.tryGitHubAlternatives();
    }
}

// AGGIUNTA: Metodo per provare URL alternativi di GitHub
async tryGitHubAlternatives() {
    const alternativeUrls = [
        'https://raw.githubusercontent.com/SiciliaHub/palermohub/main/dati-palermo-hub/palermo-hub.csv',
        'https://raw.githubusercontent.com/SiciliaHub/palermohub/master/dati-palermo-hub/palermo-hub.csv',
        'https://raw.githubusercontent.com/SiciliaHub/palermohub/PH-release-v2.0/dati-palermo-hub/palermo-hub.csv',
        // Prova anche con branch diversi se necessario
        'https://raw.githubusercontent.com/SiciliaHub/palermohub/PH-release-v2.0/palermo-hub.csv'
    ];
    
    console.log('Tentativo URL alternativi GitHub...');
    ...
    for (const url of alternativeUrls) {
        try {
            ...
            if (response.ok) {
                console.log(`✓ Successo con URL: ${url}`);
                const csvText = await response.text();
                ...
                if (typeof Papa !== 'undefined') {
                    Papa.parse(csvText, {
                        ...
                        complete: (results) => {
                            ...
                            this.buildSearchIndex();
                            this.isInitialized = true;
                            console.log(`Footer Search: Loaded ${this.allMaps.length} maps from ${url}`);
                            
                            // Debug: mostra i primi dati
                            if (this.allMaps.length > 0) {
                                console.log('Campi disponibili:', Object.keys(this.allMaps[0]));
                                console.log('Esempio di mappa:', this.allMaps[0]);
                            }
            ...
            console.log(`✗ Fallito URL: ${url} - ${error.message}`);
            continue;
        }
    }
    
    console.error('Nessun URL GitHub funziona. CSV non trovato.');
}

// CORREZIONE ANCHE per la funzione selectFooterSuggestion:
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        console.log('Campi disponibili nel dataset:', Object.keys(result.map));
        console.log('Dati completi mappa:', result.map);
        
        // Lista dei possibili campi URL da cercare
        const possibleUrlFields = [
            'url', 'URL', 'link', 'href', 'mappa', 'collegamento', 'sito', 
            'pagina', 'indirizzo', 'web', 'website', 'map', 'cartografia'
        ];
        
        let mapUrl = '';
        
        // Cerca in tutti i possibili campi (case-insensitive)
        for (const field of possibleUrlFields) {
            if (result.map[field] && result.map[field].trim() !== '') {
                mapUrl = result.map[field].trim();
                console.log(`URL trovato nel campo: ${field} = ${mapUrl}`);
                break;
            }
            
            // Prova anche versione lowercase
            const lowerField = field.toLowerCase();
            if (result.map[lowerField] && result.map[lowerField].trim() !== '') {
                mapUrl = result.map[lowerField].trim();
                console.log(`URL trovato nel campo: ${lowerField} = ${mapUrl}`);
                break;
            }
        }
        
        // Se non trova nei campi specifici, cerca qualsiasi campo con "http"
        if (!mapUrl) {
            for (const [key, value] of Object.entries(result.map)) {
                if (value && typeof value === 'string' && value.includes('http')) {
                    mapUrl = value.trim();
                    console.log(`URL trovato nel campo generico: ${key} = ${mapUrl}`);
                    break;
                }
            }
        }
        
        // Apri l'URL se trovato
        if (mapUrl) {
            // Assicurati che inizi con http/https
            if (!mapUrl.startsWith('http://') && !mapUrl.startsWith('https://')) {
                mapUrl = 'https://' + mapUrl;
            }
            console.log('Apertura URL:', mapUrl);
            window.open(mapUrl, '_blank');
        } else {
            alert(`Link non disponibile per: ${result.map.titolo || 'questa mappa'}\nControlla la console per vedere tutti i campi disponibili.`);
        }
...
// AGGIUNTA: Nuovo metodo per determinare il percorso corretto
getCorrectCSVPath() {
    ...
    // Se siamo nella root o in una pagina nella root
    if (currentDir === '/' || currentPath.endsWith('.html')) {
        return '/dati-palermo-hub/palermo-hub.csv';  // Percorso assoluto dalla root
    }
    
    // Calcola quanti livelli risalire per arrivare alla root
    const levels = (currentDir.match(/\//g) || []).length - 1;
    const relativePath = '../'.repeat(levels) + 'dati-palermo-hub/palermo-hub.csv';
    
    console.log(`Livelli da risalire: ${levels}, Percorso relativo: ${relativePath}`);
    
    return relativePath;
}

// AGGIUNTA: Metodo per tentare percorsi alternativi se il primo fallisce
async tryAlternativePaths() {
    const alternativePaths = [
        '/dati-palermo-hub/palermo-hub.csv',        // Assoluto dalla root
        './dati-palermo-hub/palermo-hub.csv',       // Relativo corrente
        '../dati-palermo-hub/palermo-hub.csv',      // Un livello su
        '../../dati-palermo-hub/palermo-hub.csv',   // Due livelli su
        '../../../dati-palermo-hub/palermo-hub.csv' // Tre livelli su
    ];
    
    console.log('Tentativo percorsi alternativi...');
    ...
    for (const path of alternativePaths) {
        try {
            ...
            if (response.ok) {
                console.log(`✓ Successo con percorso: ${path}`);
                const csvText = await response.text();
                ...
                if (typeof Papa !== 'undefined') {
                    Papa.parse(csvText, {
                        ...
                        complete: (results) => {
                            ...
                            this.buildSearchIndex();
                            this.isInitialized = true;
                            console.log(`Footer Search: Loaded ${this.allMaps.length} maps from ${path}`);
                        }
                    });
                } else {
                    this.parseCSVSimple(csvText);
            ...
            console.log(`✗ Fallito percorso: ${path} - ${error.message}`);
            continue;
        }
    }
    
    console.error('Nessun percorso alternativo funziona. CSV non trovato.');
}
