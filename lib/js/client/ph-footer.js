...
// CORREZIONE per la funzione selectFooterSuggestion
...
function selectFooterSuggestion(index) {
    if (index >= 0 && index < footerSearchSuggestions.length) {
        ...
        // Cerca l'URL in diversi possibili campi (tutti in lowercase a causa del transformHeader)
        const possibleUrlFields = [
            'url', 'URL', 'link', 'href', 'mappa', 'collegamento', 'sito', 
            'pagina', 'indirizzo', 'web', 'website', 'site', 'map', 
            'cartografia', 'visualizza', 'apri'
        ];
        
        let mapUrl = '';
        
        // Prova tutti i possibili nomi di campi
        for (const field of possibleUrlFields) {
            const fieldValue = result.map[field.toLowerCase()] || result.map[field];
            if (fieldValue && fieldValue.trim() !== '') {
                mapUrl = fieldValue.trim();
                break;
            }
        }
        
        // Debug: mostra tutti i campi disponibili se non trova l'URL
        if (!mapUrl) {
            console.log('Campi disponibili nel dataset:', Object.keys(result.map));
            console.log('Dati completi mappa:', result.map);
            
            // Fallback: cerca qualsiasi campo che contenga "http"
            for (const [key, value] of Object.entries(result.map)) {
                if (value && typeof value === 'string' && value.includes('http')) {
                    mapUrl = value.trim();
                    console.log(`URL trovato nel campo: ${key}`);
                    break;
                }
            }
        }
        ...
        if (mapUrl) {
            ...
            // Se non trova l'URL, mostra un messaggio di debug
            console.error('URL non trovato per questa mappa:', result.map.titolo);
...
// AGGIUNTA: Funzione di utilità per ispezionare i dati CSV (da aggiungere alla classe FooterSmartSearch)
// Aggiungi questo metodo alla classe FooterSmartSearch dopo buildSearchIndex():

inspectCSVData() {
    if (this.allMaps.length > 0) {
        console.log('=== ISPEZIONE DATI CSV ===');
        console.log('Numero totale mappe:', this.allMaps.length);
        ...
        const urlFields = [];
        const sampleData = this.allMaps[0];
        for (const [key, value] of Object.entries(sampleData)) {
            if (value && typeof value === 'string' && 
                (value.includes('http') || key.toLowerCase().includes('url') || 
                 key.toLowerCase().includes('link') || key.toLowerCase().includes('mappa'))) {
                urlFields.push(key);
            }
        }
        console.log('Possibili campi URL trovati:', urlFields);
        console.log('========================');
    }
}

// MODIFICA: Nel costruttore della classe FooterSmartSearch, aggiungi questa chiamata
// Modifica il metodo buildSearchIndex() per includere l'ispezione:
...
buildSearchIndex() {
    ...
    // Aggiungi l'ispezione dei dati dopo aver costruito l'indice
    this.inspectCSVData();
}
