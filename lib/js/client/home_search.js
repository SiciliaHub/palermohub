...
// AGGIUNGERE ALLA FINE DELLA FUNZIONE loadMapsData in home.js
// Sostituire la sezione complete di Papa.parse con questo:
...
complete: function(results) {
    ...
    // ✅ INTEGRAZIONE FOOTER - SINCRONIZZAZIONE FORZATA
    console.log('🔗 home.js: Avvio sincronizzazione footer...');
    
    // 1. Assicurati che window.allMaps sia impostato
    window.allMaps = allMaps;
    
    // 2. Trigger evento personalizzato
    setTimeout(() => {
        console.log('📡 home.js: Invio evento mapsDataLoaded');
    ...
    // 3. Chiamate dirette con retry multipli
    const syncFooter = () => {
        console.log('🔄 home.js: Sincronizzazione diretta footer...');
        
        // Forza assegnazione window.allMaps
        window.allMaps = allMaps;
        
        // Chiamate dirette se le funzioni esistono
        if (typeof window.updateFooterMapData === 'function') {
            console.log('✅ home.js: Chiamata updateFooterMapData');
            window.updateFooterMapData();
        }
        
        if (typeof window.loadFooterMapData === 'function') {
            console.log('✅ home.js: Chiamata loadFooterMapData');  
            window.loadFooterMapData();
        }
        
        // Verifica sincronizzazione
        setTimeout(() => {
            if (typeof window.debugFooterSearch === 'function') {
                console.log('🔍 home.js: Verifica sincronizzazione footer');
                window.debugFooterSearch();
            }
        }, 500);
    };
    
    // Esegui sincronizzazione con retry multipli
    setTimeout(syncFooter, 200);   // Primo tentativo
    setTimeout(syncFooter, 1000);  // Secondo tentativo  
    setTimeout(syncFooter, 3000);  // Terzo tentativo
    
    // 4. Notifica globale di completamento
    console.log(`🎉 home.js: Caricamento dati completato (${allMaps.length} mappe)`);
},

// AGGIUNGERE ANCHE QUESTA FUNZIONE ALLA FINE DI home.js

// Funzione per debug e verifica sincronizzazione
function verifyFooterSync() {
    console.log('🔍 home.js: Verifica sincronizzazione footer');
    console.log('- allMaps:', allMaps ? allMaps.length : 'undefined');
    console.log('- window.allMaps:', window.allMaps ? window.allMaps.length : 'undefined');
    console.log('- updateFooterMapData:', typeof window.updateFooterMapData);
    console.log('- loadFooterMapData:', typeof window.loadFooterMapData);
    console.log('- debugFooterSearch:', typeof window.debugFooterSearch);
    
    // Forza sync se dati sono disponibili
    if (allMaps && allMaps.length > 0) {
        console.log('🔄 home.js: Forzatura manuale sincronizzazione...');
...
window.verifyFooterSync = verifyFooterSync;

// Auto-verifica ogni 10 secondi per i primi 60 secondi
let verifyCount = 0;
const verifyInterval = setInterval(() => {
    verifyCount++;
    
    if (verifyCount > 6) { // Stop dopo 60 secondi
    ...
    // Se il footer non ha dati, riprova sync
    if (allMaps && allMaps.length > 0) {
        if (typeof window.updateFooterMapData === 'function') {
            window.updateFooterMapData();
        }
