...
// JAVASCRIPT COMPLETO CON CORREZIONI PAGINAZIONE E ORDINAMENTO PER DATA + ESPANSIONE RICERCA
...
// ========= RICERCA INTELLIGENTE MIGLIORATA CON ESPANSIONE =========
...
// ========= NUOVE FUNZIONI PER GESTIONE ESPANSIONE =========
...
// ========= FUNZIONI RICERCA MODIFICATE CON ESPANSIONE =========
...
function handleSmartSearchInput(event) {
    ...
    if (query.length < 2) {
        hideSuggestions();
        // NUOVO: Rimuovi classe search-active quando non c'è ricerca
        if (filtersSection) {
            filtersSection.classList.remove('search-active', 'expanded');
        }
        applyFilters();
    ...
    // NUOVO: Aggiungi classe search-active per espandere la sezione
    if (filtersSection) {
        filtersSection.classList.add('search-active', 'expanded');
    }
...
function displaySmartSuggestions(results, query) {
    ...
    // NUOVO: Assicurati che la filters-section sia espansa
    if (filtersSection) {
        filtersSection.classList.add('search-active', 'expanded');
    }
    ...
    // NUOVO: Scroll automatico alla sezione ricerca quando si espande
    setTimeout(() => {
        const filtersSection = document.querySelector('.filters-section');
        if (filtersSection) {
...
function hideSuggestions() {
    ...
    // NUOVO: Rimuovi la classe search-active solo se non c'è testo di ricerca
    const searchInput = document.getElementById('smart-search-input');
    if (searchInput && searchInput.value.trim().length < 2) {
        if (filtersSection) {
            filtersSection.classList.remove('search-active', 'expanded');
...
function applyFilters() {
    ...
    console.log("=== SORTING FILTERED MAPS ===");
    filteredMaps.sort((a, b) => {
        const dateA = parseDate(a.data || a.anno || '');
        const dateB = parseDate(b.data || b.anno || '');
        
        console.log(`Sorting comparison: "${a.data || a.anno}" vs "${b.data || b.anno}"`);
        console.log(`Parsed: ${dateA.toDateString()} (${dateA.getTime()}) vs ${dateB.toDateString()} (${dateB.getTime()})`);
        
        // Dal più recente al più vecchio
        return dateB.getTime() - dateA.getTime();
    });
    
    console.log("Top 5 after sorting:", filteredMaps.slice(0, 5).map(m => ({ 
        title: m.titolo, 
        date: m.data || m.anno,
        parsed: parseDate(m.data || m.anno || '').toDateString()
    })));
...
// ========= INIZIALIZZAZIONE EVENT LISTENERS CORRETTA CON ESPANSIONE RICERCA =========
document.addEventListener('DOMContentLoaded', function() {
    console.log('[INIT] Initializing PalermoHub with improved date sorting and search expansion');
    ...
    // Ricerca intelligente CON GESTIONE ESPANSIONE
    const searchInput = document.getElementById('smart-search-input');
    ...
    if (searchInput) {
        ...
        // NUOVO: Gestione focus/blur per l'espansione
    ...
    // NUOVO: Gestione ESC per chiudere la ricerca espansa
    ...
    console.log('[INIT] PalermoHub initialization completed with date sorting improvements and search expansion');
});